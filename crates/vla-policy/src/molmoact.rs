...
impl MolmoActPolicy {
    ...
    ) -> Result<Arc<dyn Send + Sync>, Box<dyn std::error::Error>> {
    ...
    fn generate_waypoints(&self, observation: &Observation) -> Vec<Waypoint> {
        ...
        for i in 0..self.molmo_config.num_waypoints {
            ...
            let waypoint = Waypoint {
                ...
                gripper: if i == self.molmo_config.num_waypoints - 1 {
                    1.0
                } else {
                    0.0
                },
    ...
    fn inference_local(
        ...
        let image_tensor = Tensor::from_slice(
            ...
            &[
                1,
                observation.image_shape.2 as usize,
                observation.image_shape.0 as usize,
                observation.image_shape.1 as usize,
            ],
            &Device::Cpu,
        )?;
...
impl crate::traits::Policy for MolmoActPolicy {
    ...
    fn metadata(&self) -> crate::traits::PolicyMetadata {
        crate::traits::PolicyMetadata {
            ...
            supported_actions: vec!["end_effector_delta".to_string(), "gripper".to_string()],
        }
    }
...
mod tests {
    use super::*;
    use crate::{ActionHead, DeviceConfig, NormalizationConfig, Observation, PolicyConfig};
